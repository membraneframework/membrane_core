defmodule Membrane.Integration.AutoDemandsTest do
      ...
      pipeline =
        Pipeline.start_link_supervised!(
          structure: [
            ...
            |> reduce_link(1..filters, &to_new(&1, {:filter, &2}, filter))
            |> to_new(:sink, Sink)
          ]
        )
  ...
  test "buffers pass through auto-demand tee" do
    ...
    pipeline =
      Pipeline.start_link_supervised!(
        structure: [
          spawn_child(:source, %Source{output: 1..100_000}) |> to_new(:tee, AutoDemandTee),
          link(:tee) |> to_new(:left_sink, Sink),
          link(:tee) |> to_new(:right_sink, %Sink{autodemand: false})
        ]
      )
    ...
    pipeline =
      Pipeline.start_link_supervised!(
        structure: [
          ...
          |> to_new(:filter, AutoDemandFilter)
          |> to_new(:sink, Sink)
        ]
      )
    ...
    pipeline =
      Pipeline.start_supervised!(
        structure: [
          spawn_child(:source, PushSource)
          |> to_new(:filter, AutoDemandFilter)
          |> to_new(:sink, %Sink{autodemand: false})
        ]
      )
