defmodule Membrane.PipelineSpec do
  ...
  describe ".start_link/3" do
    context "when starting `TrivialPipeline`" do
      ...
      it "should return an ok result" do
        expect(described_module().start_link(module(), options(), process_options())).to be_ok_result()
      end
    ...
    context "when starting module that is not a pipeline" do
      ...
      it "should return an error result" do
        expect(described_module().start_link(module(), options(), process_options())).to be_error_result()
      end
  ...
  describe ".start/3" do
    context "when starting `TrivialPipeline`" do
      ...
      it "should return an ok result" do
        expect(described_module().start(module(), options(), process_options())).to be_ok_result()
      end
    ...
    context "when starting module that is not a pipeline" do
      ...
      it "should return an error result" do
        expect(described_module().start(module(), options(), process_options())).to be_error_result()
      end
    ...
    context "when receiving message from the element" do
      ...
      let :child_pid, do: self()
        ...
        it "should invoke handle_message from pipeline module with correct arguments" do
          described_module().handle_info(message(), state())
          expect(module()).to accepted(:handle_message, [internal_message(), child_pid(), internal_state()])
        end
