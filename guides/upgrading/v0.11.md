...
Change arguments in `handle_element_start_of_stream` and `handle_element_end_of_stream`, since till now on they accept children name and pad reference as two separate arguments instead of storing them as tuple in the first argument.
...
Rename `handle_notification/4` into `handle_child_notification/4` in pipelines and bins
...
Add an implementation of `handle_parent_notification/3` in elements and bins in place of the `handle_other/3` that were responsible for receiving messages from the parents.
...
* `Membane.ParentSpec.to/3` -> `Membrane.ChildrenSpec.child/4`
