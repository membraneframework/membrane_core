defmodule Membrane.Core.Element.OwnDemandHandler do
  alias Membrane.Core
  ...
  def handle_own_demand(pad_name, source, :set, buf_cnt, state) do
    {:ok, state} = set_sink_own_demand(pad_name, source, buf_cnt, state)

    supply_own_demand(pad_name, source, buf_cnt, state)
  end

  def handle_own_demand(pad_name, :self, :increase, buf_cnt, state) do
    ...
    supply_own_demand(pad_name, :self, total_buf_cnt, state)
  end

  def check_and_handle_demands(pad_name, state) do
    demand = PadModel.get_data!(pad_name, :own_demand, state)

    own_demand_result =
      if demand > 0 do
        supply_own_demand(pad_name, :self, demand, state)
      else
        {:ok, state}
      end

    with {:ok, %State{type: :filter} = state} <- own_demand_result do
  ...
  defp supply_own_demand(pad_name, source, buf_cnt, state) do
    pb_output =
      PadModel.get_and_update_data(
        pad_name,
        :buffer,
  ...
  defp do_handle_pullbuffer_output(
    ...
    {:ok, state} = update_sink_own_demand(pad_name, source, &{:ok, &1 - buf_cnt}, state)

    BufferController.exec_buffer_handler(pad_name, source, buffers, state)
  end

  defp set_sink_own_demand(pad_name, :self, size, state),
    do: PadModel.set_data(pad_name, :own_demand, size, state)

  defp set_sink_own_demand(_pad_name, _src, _f, state), do: {:ok, state}

  defp update_sink_own_demand(pad_name, :self, f, state),
    do: PadModel.update_data(pad_name, :own_demand, f, state)

  defp update_sink_own_demand(_pad_name, _src, _f, state), do: {:ok, state}
end
